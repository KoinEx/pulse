...
export default class EosService {
  // service to access all the eos blockchain functions
  ...
  async initialize(passkey = null,contract=null){ // to initialize the eos object
    ...
    if(!CUSTOM_NETWORKS[this.networkType]) { // to check if the current network is a custom network or default one and then initialize the config accordingly
    ...
    if (passkey) {
      ...
      localConfigForEos.httpEndpoint = null; // to just sign the transaction and not pushing to the blockchain
  ...
  createAccount(name,deviceID,memo){ // to create new account on the blockchain
    return new Promise(async (resolve,reject)=>{
      ...
      this.ecc.randomKey().then( async (key) => {
        ...
        const [error] = await to(this.getAccountByName({account_name:name})); // its a check whether account is already present with this name or not
        if(error){
  ...
  async getAccountByName({account_name}){  // to get an account details using  name from blockchain
  ...
  pushTransaction(transctionData){  // to push transaction through background script
    ...
    return new Promise((resolve,reject)=>{
      port.onMessage.addListener((result)=>{ // listener for the response from the background script
  ...
  async verifyTxn(txnID){ // to verify the transaction for creating account on mainnet
  ...
  async customActions(contract,from,action,passKey,data){ // to handle custom actions of contracts
    try{
  ...
  async delegate(password,decimalPrecision,{from,receiver,CPU_quantity,net_quantity,symbol}){ // to delegate the CPU
